...
int main(int argc, char** argv) {
    ...
    for(EBCFiles::iterator i = ebcFiles.begin(); i != ebcFiles.end(); i++) {
        ...
        if(color.w()<1){
            group->addChild(node, true, (flag == 0 ? true : false)); // true = render as transparent, flag=1:render as two-sided
        }
        else{
            group->addChild(node, false, (flag == 0 ? true : false)); // false = hint that it's not transparent, flag=1:render as two-sided
        }
        node->setColor(color);
        id++;
    }
